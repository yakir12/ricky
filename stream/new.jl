...
struct DetectoRect
    ...
    rect::MVector{4, Int}
    bee::Ref{Bee}
    DetectoRect(id) = new(id, AprilTagDetector(), MVector(1, 1, sz...), Ref{Bee}(Bee(time_ns(), missing)))
end

Base.close(d::DetectoRect) = freeDetector!(d.detector)

function (d::DetectoRect)(buff, ts::UInt64)
    r1, c1, r2, c2 = d.rect
    cropped = buff[r1:r2, c1:c2]
    ...
    for tag in tags
        if tag.id == d.id
            c0 = SV(reverse(tag.c))
            c = c0 + SV(r1, c1)
            d.rect[1:2] .= max.(1, round.(Int, c .- d.min_radius::Float64))
            d.rect[3:4] .= min.(d.h, round.(Int, c .+ d.min_radius::Float64))
            d.bee[] = c
            return nothing
        end
    end
